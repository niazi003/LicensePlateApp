...
import { View, Text, FlatList, TouchableOpacity, StyleSheet, TextInput, Image, Modal } from 'react-native';
...
const HomePage = () => {
    ...
    useEffect(() => {
        // Keep store up to date for detail screens etc., but do not auto-render list
                ...
                renderItem={({ item }) => (
                    ...
                    >
                        <View style={{flexDirection: 'row', alignItems: 'center'}}>
                            {item.images ? (
                                <Image source={{ uri: item.images }} style={styles.thumb} />
                            ) : (
                                <View style={[styles.thumb, {backgroundColor: '#eee'}]} />
                            )}
                            <View style={{marginLeft: 10, flex: 1}}>
                                <Text style={{fontWeight: '600'}}>{item.name}</Text>
                                <Text style={{color: '#666'}}>{item.state}, {item.country}  ·  {item.external_id || '-'}</Text>
                            </View>
                        </View>
                    </TouchableOpacity>
                )}
