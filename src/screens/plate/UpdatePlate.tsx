...
const UpdatePlate = ({ route }: Props) => {
  ...
  const composeFeaturesTags = (): string => {
    const metas = [font ? `font=${font}` : null, stateLocation ? `state_location=${stateLocation}` : null].filter(Boolean) as string[];
  ...
  useEffect(() => {
    if (plate) {
      ...
      // hydrate UI meta from tags if present
      const fontMatch = /(?:^|;)font=([^;]+)/.exec(plate.features_tags || '');
      const locMatch = /(?:^|;)state_location=([^;]+)/.exec(plate.features_tags || '');
      if (fontMatch) setFont(fontMatch[1] as any);
      if (locMatch) setStateLocation(locMatch[1] as any);
      if (plate.all_colors) setSelectedColors(plate.all_colors.split(',').map(s => s.trim()).filter(Boolean));
    }
  }, [plate]);
