    ...
    <ScrollView style={styles.container}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 12 }}>
        {patterns.map(p => (
          <TouchableOpacity
            key={p.pattern_id}
            style={[styles.pill, patternChoice === p.pattern && styles.pillActive]}
            onPress={() => setPatternChoice(p.pattern)}
          >
            <Text
              style={[styles.pillText, patternChoice === p.pattern && styles.pillTextActive]}
            >
              {p.pattern}
            </Text>
          </TouchableOpacity>
